...
// Memory optimization
//
// Is offloading the computation to a CUDA or OpenCL device worthwhile?
//
// No, because the transfer time of the data to and from the device are
// higher than just to calculate it on the CPU without any data transfer.
//
...
int main (int argc, char **argv)
{
  ...
  int n_sectors, sector_size, stride;

  n_sectors = N_SECTORS;
  sector_size = SECTOR_SIZE;
  blockDimX = strtol(argv[3],NULL,0);                                               // optimize!
  blockDimY = strtol(argv[4],NULL,0);                                             // optimize!
  blockDimZ = 1;                                               // optimize!
  gridDimX = strtol(argv[1],NULL,0);                                                // optimize!
  gridDimY = strtol(argv[2],NULL,0);                          // optimize!
  gridDimZ = 1;
                                                  // optimize!
  ...
  for (i = 0; i < N_SECTORS; i++)
    modify_sector_cpu_kernel (&host_sector_data[i], host_sector_number[i], n_sectors, sector_size, stride);
  //printf("The cpu kernel took %.3e seconds to run (single core)\n",get_delta_time ());
  printf("%.3e", get_delta_time());
  // compare
  for(i = 0; i < (int) sector_data_size / (int) sizeof (unsigned int); i++)
                                      ...
                                      unsigned int sector_size, unsigned int stride)
{
 ...
 // modify the sector data
  ...
  { x = a * x + c;                            // update the pseudo-random generator state
    sector_data[i * stride] ^= x;                      // modify the sector data
  }
}
